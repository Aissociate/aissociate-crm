/*
  # 21 — Fiche de suivi contact (prospect/apprenant) + journal d'actions

  D'après la feuille de suivi prospect : enrichit `contacts` (qualification,
  suivi commercial, checklist conformité, commissions) et ajoute un journal
  d'actions `contact_actions` (suivi des actions, dernière/prochaine).

  La fiche affiche aussi le statut transverse (pipeline, dossiers, sessions)
  via les relations existantes (opportunites/dossiers/session_participants).
*/

-- Identification / infos / qualification
...
create table if not exists public.contact_actions (
  ...
  type        text not null default 'note',   -- appel | email | rdv | relance | note | autre
  description text not null,
  faite       boolean not null default true,   -- false = action planifiée (à faire)
...
-- Accès aligné sur la visibilité du contact parent
