/*
  ...
  Reprend les actualités du blog de la semaine écoulée dans un e-mail unique et
  soigné, envoyé à toute la base de contacts. Brouillon par défaut (revue par la
  direction), puis envoi automatique si activé.

  - Table `newsletters` : éditions générées (HTML, période, statut, stats).
  - Paramètre `newsletter` : éditorial, expéditeur, auto-envoi, RGPD.
  - Cron `newsletter-weekly` : appelle l'Edge Function `generate-newsletter`.
*/
...
-- Réservé à la direction (génération, revue, envoi).
...
-- Configuration par défaut (éditée dans Paramètres › Newsletter).
...
-- Cron hebdomadaire (lundi 9h) — bloc défensif (Vault + fonction requis).
do $$
begin
