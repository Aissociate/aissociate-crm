/*
  ...
  - `formateurs` : intervenants (identité, spécialités, statut)
  - `formateur_documents` : pièces afférentes (CV, diplômes, contrats, habilitations)
  - `sessions_formation.formateur_id` : affectation d'un formateur à une session
  - bucket Storage privé `formateurs` pour les fichiers

  Accès : lecture pour tous les authentifiés ; écriture réservée aux managers
  (admin / directeur_commercial). L'affectation au planning se fait via la
  session (éditable par tous).
*/
...
create table if not exists public.formateur_documents (
  ...
  titre        text not null,
  categorie    text not null default 'autre', -- cv | diplome | contrat | habilitation | autre
  fichier_url  text,
...
-- Affectation d'un formateur à une session du planning
...
-- ── Storage : bucket privé pour la documentation formateur ──
