/*
  ...
  - contact_requests : depot des demandes des formulaires publics (site OF).
    Insertable par un visiteur anonyme ; lisible/gerable par la direction cote CRM.
  - Trigger : chaque lead cree automatiquement un CONTACT (type prospect) dans le CRM.

  Securite : l'insertion publique est limitee a cette table de depot ; la creation
  du contact se fait via une fonction SECURITY DEFINER.
*/
...
drop policy if exists contact_requests_manage_update on public.contact_requests;
create policy contact_requests_manage_update on public.contact_requests
  for update to authenticated using (is_manager()) with check (is_manager());

drop policy if exists contact_requests_manage_delete on public.contact_requests;
create policy contact_requests_manage_delete on public.contact_requests
