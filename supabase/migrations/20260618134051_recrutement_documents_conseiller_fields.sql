/*
  # Coffre-fort de recrutement + champs conseiller (ticket Benjamin)

  Ticket 6 — stockage de documents dès le recrutement, transférables au conseiller.
  Ticket 7 — statut du conseiller (en cours / inactif / ancien) + date de recrutement.

  - Table `candidat_documents` (métadonnées + URL) — réservée à la direction.
  - Bucket privé `recrutement` (URL signée à l'ouverture, direction uniquement).
  - Colonnes `statut_conseiller` et `date_recrutement` sur `profiles`.
*/
...
-- Recrutement réservé à la direction (is_manager).
