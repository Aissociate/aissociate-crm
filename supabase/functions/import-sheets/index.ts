// Supabase Edge Function — import des Google Sheets (candidatures + prospects)
// Déploiement : supabase functions deploy import-sheets
//
// Sources (Google Sheets publiés en lecture « toute personne avec le lien ») :
//   - Candidatures « Chargé de formation »  -> table candidats
//   - Prospects en cours                     -> table contacts (type prospect)
// Les IDs par défaut ci-dessous peuvent être surchargés par les secrets
//   SHEET_CANDIDATS_ID / SHEET_PROSPECTS_ID.
//
// Appel : supabase.functions.invoke('import-sheets', { body: { source: 'all' | 'candidats' | 'prospects' } })
...
async function fetchSheet(id: string): Promise<Row[]> {
  ...
  return parse(text, { columns: true, skip_empty_lines: true, relax_column_count: true, trim: true }) as Row[];
}
...
// Concatène les colonnes "non structurées" en notes (clé: valeur)
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (source === 'all' || source === 'candidats') {
      ...
      const payloads = rows
        .filter((r) => r.id && r.full_name && !r.full_name.startsWith('<') && !/test lead/i.test(JSON.stringify(r)))
        .map((r) => {
      ...
      let imported = 0;
      if (payloads.length) {
        const { error, count } = await sb.from('candidats')
    ...
    if (source === 'all' || source === 'prospects') {
      ...
      const payloads = rows
        ...
        .map((r) => {
          const { prenom, nom } = splitName(r.full_name);
          const entête: string[] = [];
          if (r.company_name) entête.push(`Entreprise : ${r.company_name}`);
          if (r.ville) entête.push(`Ville : ${r.ville}`);
          const commentaires = notesFrom(r, skip);
          const notes = [entête.join('\n'), commentaires].filter(Boolean).join('\n');
