  ...
  const syncInbox = async () => {
    ...
    const timeoutPromise = new Promise<{ data: null; error: { message: string } }>((resolve) => {
      timeoutId = setTimeout(
        () => resolve({ data: null, error: { message: '__timeout__' } }),
        90000,
      );
    });

    const raw = await Promise.race([
      supabase.functions.invoke('fetch-emails').then((r) => { clearTimeout(timeoutId); return r; }),
      timeoutPromise,
    ]);

    setSyncing(false);

    const { data: res, error } = raw as { data: unknown; error: unknown };
    if (error) {
      const err = error as Record<string, unknown>;
      if (err.message === '__timeout__') {
        alert("La synchronisation a expiré (90 s). Vérifiez la configuration IMAP (hôte/port/identifiants) dans Paramètres.");
        return;
      }
      // Extraire le vrai message depuis le corps JSON de la réponse 500
      let msg = (err.message as string) ?? 'Erreur inconnue';
      try {
        const body = await (err.context as Response | undefined)?.json();
        if (body?.error) msg = body.error;
      } catch { /* ignore */ }
      alert(`Réception IMAP échouée :\n${msg}`);
      return;
    }

    const n = res as { imported?: number; error?: string } | null;
    if (n?.error) { alert(`Réception IMAP : ${n.error}`); return; }
    setTab('entrant');
    refresh();
