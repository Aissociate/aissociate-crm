...
export default function Kanban() {
  ...
  return (
    <div>
      <PageHeader title="Kanban" subtitle={board?.description ?? 'Suivi visuel des dossiers (4.6)'} />
      {!board ? (
            ...
            return (
              <div key={col.id} className="flex w-72 shrink-0 flex-col">
                <div className="mb-2 flex items-center justify-between px-1">
                  ...
                  <button onClick={() => openAdd(col.id)} className="rounded p-1 text-slate-400 hover:bg-slate-200"><Plus className="h-4 w-4" /></button>
                </div>
                    ...
                    return (
                      <div key={carte.id} className="card p-3">
                        <div className="flex items-start justify-between gap-2">
                          <p className="text-sm font-medium text-slate-900">{carte.titre}</p>
                          <button onClick={() => removeCarte(carte)} className="rounded p-0.5 text-slate-300 hover:text-red-600"><Trash2 className="h-3.5 w-3.5" /></button>
                        </div>
                        {carte.description && <p className="mt-1 text-xs text-slate-500">{carte.description}</p>}
                        {carte.echeance && <p className="mt-1 text-xs text-amber-600">Échéance : {formatDate(carte.echeance)}</p>}
                        <div className="mt-2 flex justify-between">
                          <button disabled={idx === 0} onClick={() => moveCarte(carte, -1)} className="rounded p-1 text-slate-400 hover:bg-slate-100 disabled:opacity-30"><ChevronLeft className="h-4 w-4" /></button>
                          <button disabled={idx === colonnes.length - 1} onClick={() => moveCarte(carte, 1)} className="rounded p-1 text-slate-400 hover:bg-slate-100 disabled:opacity-30"><ChevronRight className="h-4 w-4" /></button>
                        </div>
                      </div>
                    );
                  })}
